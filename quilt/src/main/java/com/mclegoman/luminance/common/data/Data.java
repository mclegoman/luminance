...
public class Data {
	public static final Version version = Version.parse(QuiltLoader.getModContainer("luminance").get().metadata(), "EBTw0O1c");
	...
	public static boolean isModInstalledVersionOrHigher(String modId, String requiredVersion, boolean substring, String separator) {
		try {
			if (isModInstalled(modId)) {
				...
				if (modContainer.isPresent()) return checkModVersion(modContainer.get().metadata().version().raw(), requiredVersion, substring);
			}
